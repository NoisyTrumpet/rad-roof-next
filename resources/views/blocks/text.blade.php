...
<div id="{{ $block['id'] }}" class="{{ $block['classes'] }}" style="background-color:@isfield('background_color', 'secondary') #ccaf88 !important @endfield @isfield('background_color', 'primary') #2f372b !important @endfield @isfield('background_color', 'light') #f0f0f0 !important @endfield @isfield('background_color', 'none') transparent !important @endfield;">
    <div class="container-fluid @isfield('variants', 'construction')py-5 @else py-3 @endfield px-md-5 px-3 text-color-@field('background_color')" style="@isfield('variants', 'construction')max-width: 1000px;@endfield">
        <div class="text-block-wrapper @isfield('variants', 'construction')@else pb-5 @endfield border-@field('variants')" style="@isfield('has_angle', 'bottom')padding-bottom: 0 !important;@endfield">
            <div class="text-block">
                <h2 class="@isfield('variants', 'construction')mb-3 @endfield" style="text-align:{{$alignTitle}}">{{ $title }}</h2>
                <div>
                    {!! $content !!}
                </div>
                <!-- {{ $variants }} -->
